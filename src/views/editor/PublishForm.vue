...
import { createChannel as cChannel, deleteChannel as dChannel } from '@/api/modules/channel'
...
export default defineComponent({
  ...
  setup() {
    ...
    const createChannel = async () => {
      ...
      try {
        await validate()
        const { id } = await cChannel(payload)
        store.channels = [...store.channels, { ...payload, id }]
        form.channelName = ''
      }
      catch (e) {
        console.error(e)
    ...
    const deleteChannel = (id: string) => {
      dChannel(id)
      store.channels = store.channels.filter(el => el.id !== id)
    }
    onMounted(() => {
          ...
          <a-tab-pane key="channels" tab="发布为作品">
            <a-row v-for="channel in channels" :key="channel.id" class="channel-item">
              ...
              <a-col :span="18" class="left-gap">
                ...
                <a-row>
                  <a-col :span="18">
                    <a-input :id="`channel-url-${channel.id}`" :value="generateChannelURL(channel.id)" :readonly="true" />
                  </a-col>
                  <a-col :span="6">
                    <a-button class="copy-button" :data-clipboard-target="`#channel-url-${channel.id}`">
                      复制
                    </a-button>
                  </a-col>
                </a-row>
              ...
              <div class="delete-area">
                <a-button type="danger" size="small" :disabled="deleteDisabled" @click="deleteChannel(channel.id)">
                  删除渠道
                </a-button>
              </div>
            </a-row>
